...
type <- c(unique(test_dat$subjectID))
typea <- seq(0,0, length.out = length(type))
type <- cbind(type,typea)
type2 <- c(unique(train_dat$subjectID))
typeb <- seq(1,1,length.out=length(type2))
type2 <- cbind(type2,typeb)
type_key <- rbind(type,type2)
colnames(type_key) <- c("subjectID","type")
